...
class AuthorController extends Controller
{
    ...
    function actionReport($mode = 'customer', $limit = 3)
    {

        switch ($mode) {
            case "customer":
            default:
                $items = Author::model()->with(
                    ['books' => ['scopes' => 'hasCustomers', 'group' => 'books_books.author_id, bookCustomers.book_id']]
                )->findAll(['having' => 'COUNT(DISTINCT customer_id) >= ' . $limit]);

                $title = "Authors that written a books with minimum {$limit} readers";
        }
